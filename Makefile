...
trtpro_inc := /datav/software/anaconda3/lib/python3.9/site-packages/trtpy/include
trtpro_lib := /datav/software/anaconda3/lib/python3.9/site-packages/trtpy/trt8cuda112cudnn8/py39
...
link_opencv    := opencv_core opencv_imgproc opencv_imgcodecs opencv_calib3d opencv_features2d opencv_flann opencv_photo
link_ceres     := ceres protobuf cxsparse lapack cholmod
link_ceres_dependency := gflags glog
...
link_librarys  := $(link_opencv) $(link_cuda) $(link_tensorRT) $(link_sys) $(link_trtpro) $(link_ceres) $(link_ceres_dependency)
...
include_eigen     := /usr/include/eigen3 
include_ceres     := /usr/local/include

lib_all       := $(cuda_home)/lib64 $(trtpro_lib) $(syslib)
lib_opencv    := $(cpp_pkg)/opencv4.2/lib
lib_ceres     := /usr/local/lib
...
library_paths := $(lib_all) $(lib_opencv) $(lib_ceres)
...
cpp_compile_flags := -std=$(stdcpp) -w -g -O0 -m64 -fPIC -fopenmp -pthread $(defined) -Wfatal-errors
